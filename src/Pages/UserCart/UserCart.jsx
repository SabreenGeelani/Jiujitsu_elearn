...
const UserCart = () => {
  ...
  return (
    <>
      {isLoading ? <SyncLoader id="spinner-usercourseview" size={8} color="black" /> : 
            ...
            {cartItems?.cart?.map((item, index) => (
              <div key={index} className='mid-left-cards-usercart'>
                <div className="mid-left-left-usercart">
                  ...
                  <div style={{cursor:"pointer"}} onClick={()=> handleRemoveCart(item?.course_id)}>
                    <p>Remove</p>
                  </div>
                </div>
                <div className="mid-left-mid-usercart">
                  <h5>{item?.title?.split(' ').slice(0, 3).join(' ') + "..."}</h5>
                  <p>{item?.description?.split(' ').slice(0, 7).join(' ') + "..."}</p>
              ...
              <div className="card-bottom-usercart" key={index}>
                ...
                <div className="bottom-card-userusercart">
                  ...
                  <div onClick={() => handleCart(items?.id)}>
                      ...
                      {loadingItems[items?.id] ? <PulseLoader size={8} color="white"/> : <h6>Add to Cart</h6>}
